...
Atualizado até a **aula 1** da UC4. Salve na raiz do seu repositório com o nome `REGRAS.md`,
...
- Ambiente virtual `venv` e instalação com `pip install "fastapi[standard]"`.
- `FastAPI()`, o decorador `@app.get` e função de rota com `def` comum.
- Retorno de dicionário ou de lista, que o FastAPI converte em JSON sozinho.
- Servidor com `fastapi dev main.py` e a documentação automática em `/docs`.
...
- `APIRouter`, separação em camadas, `Depends`, arquivo `.env`.
