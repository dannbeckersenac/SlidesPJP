...
const pausas = PAUSAS.trim().split("\n").map(linha => {
  ...
  const [, dia, resto] = linha.trim().match(/^(\S+)\s+(.*)$/);
  const [tipo, nome] = resto.split("|");
  return { data: `${ANO}-${dia}`, tipo, nome };
});
const pausaPorData = Object.fromEntries(pausas.map(p => [p.data, p]));

/* visitas: dias que continuam sendo aula, só que em outro endereço */
const visitas = VISITAS.trim().split("\n").filter(Boolean).map(linha => {
  const [, dia, resto] = linha.trim().match(/^(\S+)\s+(.*)$/);
  const [local, nota] = resto.split("|");
  return { data: `${ANO}-${dia}`, local, nota };
});
...
const visitaHoje = visitaPorData[hojeISO];
...
if (aulaHoje && visitaHoje) {
  rotuloHoje.textContent = "Ver hoje no calendário";
  notaHoje.textContent = `Hoje é ${visitaHoje.local}: ${visitaHoje.nota}.`;
} else if (aulaHoje) {
  rotuloHoje.textContent = "Ver hoje no calendário";
...
for (let m = MES_INICIAL; m <= MES_FINAL; m++) {
  ...
  for (let d = 1; d <= totalDias; d++) {
    ...
    const visita = visitaPorData[iso];
...
cartaoMarcos("Encontros fora do Senac", visitas.map(v => ({
  data: diaMes(v.data), titulo: v.local, sub: v.nota
})));
...
function selecionar(iso, opcoes = {}) {
  ...
  if (aula && visita) {
    detalhe.style.setProperty("--c", "var(--visita)");
    dTitulo.textContent = visita.local;
    dSub.textContent =
      `${UC[aula.uc].sigla} ${aula.n} de ${UC[aula.uc].total} · 4 horas · ${visita.nota}`;
  } else if (aula) {
